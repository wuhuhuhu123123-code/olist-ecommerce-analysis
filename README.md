...
- **未分类商品占营收 1.29%**,品类表覆盖营收约 98.6%,不影响 Top10 结论。
...
写 SQL 时踩到的两个真实错误,留在代码注释里:

1. **复购率算出 0%**:一开始按 `orders.customer_id` 分组统计复购,结果恒为 0。原因是 Olist 的 `customer_id` **每单唯一**,真正标识"人"的是 `customers.customer_unique_id`。改用后者后得到 3.0%——**这个字段陷阱会让所有客户级分析失效**。
2. **品类营收与总 GMV 对不上**:品类查询漏了 `order_status='delivered'` 过滤,算出的总额比总 GMV 高 2.8%,且第 7/8 名排序会变。**多表查询必须对齐口径**。
